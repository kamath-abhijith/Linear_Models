...
LOGISTIC REGRESSION ON TOY 2D DATA
...
parser = argparse.ArgumentParser(
    description = "LOGISTIC REGRESSION ON 2D TOY DATA"
)
...
if os.path.isfile(path + 'model_LOG_dataset_' + dataset + '_size_' + \
    str(training_size) + '.pkl') and force_train==False:
    
    print('PICKING PRETRAINED MODEL')
    
    f = open(path + 'model_LOG_dataset_' + dataset + '_size_' + \
    ...
    f = open(path + 'model_LOG_dataset_' + dataset + '_size_' + str(training_size) + '.pkl', 'wb')
    pickle.dump(model, f)
    f.close()
...
path = './../results/ex1/'
save_res = path + 'samples_LOG_dataset_' + dataset + '_size_' + str(training_size)
