...
LINEAR LEAST-SQUARES CLASSIFICATION ON TOY 2D DATA
...
parser = argparse.ArgumentParser(
    description = "LEAST-SQUARES CLASSIFICATION ON 2D TOY DATA"
)

parser.add_argument('--dataset', help="Dataset for training and testing", default='Gamma')
...
if os.path.isfile(path + 'model_LS_dataset_' + dataset + '_size_' + \
    str(training_size) + '.pkl') and force_train==False:
    
    print('PICKING PRETRAINED MODEL')
    
    f = open(path + 'model_LS_dataset_' + dataset + '_size_' + \
    ...
    f = open(path + 'model_LS_dataset_' + dataset + '_size_' + str(training_size) + '.pkl', 'wb')
    pickle.dump(model, f)
    f.close()
...
# PREDICTIONS ON MESH
num_samples = 500
x1, x2 = np.meshgrid(np.linspace(-1, 20, num_samples), \
    np.linspace(-1, 20, num_samples))
mesh_points = np.array([x1, x2]).reshape(2, -1).T

mesh_samples = features.transform(mesh_points)
mesh_labels = model.predict(mesh_samples).reshape(num_samples, num_samples)

# %% PLOT SAMPLES AND CONFUSION MATRIX

os.makedirs('./../results/ex1', exist_ok=True)
path = './../results/ex1/'
save_res = path + 'samples_LS_dataset_' + dataset + '_size_' + str(training_size)
...
save_res = path + 'acc_LS_dataset_' + dataset + '_size_' + str(training_size)
